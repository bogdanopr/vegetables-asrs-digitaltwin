import { useEffect, useRef, useState } from 'react';
import { useStore, type StoreState } from '../../store/useStore';
import { ChevronUp, ChevronDown, Terminal } from 'lucide-react';

export const DataFeed = () => {
    const logs = useStore((state: StoreState) => state.logs);
    const endRef = useRef<HTMLDivElement>(null);
    const [isMobile, setIsMobile] = useState(window.innerWidth < 768);
    const [isCollapsed, setIsCollapsed] = useState(window.innerWidth < 768);

    useEffect(() => {
        const handleResize = () => {
            const mobile = window.innerWidth < 768;
            setIsMobile(mobile);
            // Auto-collapse only on mobile transition
            if (mobile) setIsCollapsed(true);
        };
        window.addEventListener('resize', handleResize);
        return () => window.removeEventListener('resize', handleResize);
    }, []);

    // Auto-scroll to bottom of feed
    useEffect(() => {
        if (!isCollapsed) {
            endRef.current?.scrollIntoView({ behavior: 'smooth' });
        }
    }, [logs, isCollapsed]);

    return (
        <div style={{
            position: 'absolute',
            left: isMobile ? (isCollapsed ? 'auto' : 10) : 20,
            right: isMobile ? 10 : 'auto',
            top: isMobile ? 'auto' : 100,
            bottom: isMobile ? (isCollapsed ? 10 : 40) : 40,
            width: isMobile ? (isCollapsed ? 160 : 'calc(100% - 20px)') : 350,
            height: isMobile ? (isCollapsed ? 36 : 200) : 'auto',
            maxHeight: isMobile ? '40vh' : 'calc(100% - 140px)',
            pointerEvents: 'auto',
            background: 'rgba(0, 0, 0, 0.85)',
            backdropFilter: 'blur(8px)',
            borderRadius: 12,
            border: '1px solid rgba(0, 255, 0, 0.2)',
            display: 'flex',
            flexDirection: 'column',
            overflow: 'hidden',
            fontFamily: '"Fira Code", monospace',
            boxShadow: '0 0 20px rgba(0, 255, 0, 0.1)',
            transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
            zIndex: 100
        }}>
            {/* Header */}
            <div
                onClick={() => isMobile && setIsCollapsed(!isCollapsed)}
                style={{
                    padding: isMobile ? '8px 12px' : '10px 15px',
                    borderBottom: isCollapsed ? 'none' : '1px solid rgba(0, 255, 0, 0.2)',
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'space-between',
                    gap: 8,
                    background: 'rgba(0, 255, 0, 0.05)',
                    cursor: isMobile ? 'pointer' : 'default'
                }}
            >
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <Terminal size={14} color="#4caf50" />
                    <span style={{ color: '#4caf50', fontSize: isMobile ? 10 : 12, textTransform: 'uppercase', letterSpacing: 1, fontWeight: 'bold' }}>
                        {isMobile && isCollapsed ? 'Logs' : 'System_Log::Stream'}
                    </span>
                </div>
                {isMobile && (
                    <div style={{ color: '#4caf50' }}>
                        {isCollapsed ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                    </div>
                )}
            </div>

            {/* Logs Area */}
            {!isCollapsed && (
                <div style={{
                    flex: 1,
                    overflowY: 'auto',
                    padding: isMobile ? '8px 12px' : '15px',
                    display: 'flex',
                    flexDirection: 'column',
                    gap: 6
                }}>
                    {logs.slice().reverse().map((log: string, i: number) => (
                        <div key={i} style={{
                            fontSize: isMobile ? 9 : 11,
                            lineHeight: '1.4',
                            color: log.includes('Error') ? '#ff4444' : '#e0e0e0',
                            display: 'flex',
                            gap: 6
                        }}>
                            <span style={{ color: '#666', minWidth: 20 }}>[{i + 1}]</span>
                            <span>{log}</span>
                        </div>
                    ))}
                    <div ref={endRef} />
                </div>
            )}

            {/* Footer Status */}
            {!isMobile && !isCollapsed && (
                <div style={{
                    padding: '6px 15px',
                    borderTop: '1px solid rgba(255,255,255,0.05)',
                    fontSize: 10,
                    color: '#666',
                    display: 'flex',
                    justifyContent: 'space-between'
                }}>
                    <span>STATUS: ONLINE</span>
                    <span>BAUD: 9600</span>
                </div>
            )}
        </div>
    );
};
